...
if ( ! class_exists( 'BravePop_ConstantContact' ) ) {
   ...
   class BravePop_ConstantContact {
      ...
      public function add_to_lists($email, $list_id, $fname='', $lname='', $phone=''){
         ...
         $getResponse = wp_remote_get( 'https://api.constantcontact.com/v2/contacts?email='.$email.'&api_key='.$this->api_key, array('method' => 'GET','headers' => array( 'Authorization' => 'Bearer ' . $this->access_key)) );
         ...
         if($getData && isset($getData->results) && is_array($getData->results) && isset($getData->results[0])  && isset($getData->results[0]->id)){
            ...
            $putArgs = array('method' => 'PUT','headers' => array( 'content-type' => 'application/json', 'Authorization' => 'Bearer ' . $this->access_key), 'body' => json_encode($updatedContact)  );
