...
if ( ! class_exists( 'BravePop_Element_Products' ) ) {
   ...
   class BravePop_Element_Products {
      ...
      public function render_css() { 
         ...
         return  $elementInnerStyle . $elementContentWStyle . $elementPostStyle .$elementButtonStyle .$elementTitleStyle .$elementImageStyle .$elementPriceStyle .$elementChunkStyle . $elementSliderNavStyle .$elementSliderNavActiveStyle .$elementRibonStyle .$elementRibonBGStyle .$elementContentStyle ;
               ...
               $postHTML .= '<div class="brave_product__chunk '.$slideClass. $hasBackgroundClass. $buttonAlwaysClass.'">';
                     foreach ($block as $theproduct) {
                        ...
                        $postHTML .=  '<div id="brave_product-'.$productID.'" class="brave_product">';
                           ...
                           $cartButton = '<a href="?add-to-cart='.$productID.'" '.$goalAction.' data-quantity="1" class="add_to_cart_button ajax_add_to_cart" data-product_id="'.$productID.'" data-product_sku="'.$productSKU.'" rel="nofollow">'.$cartIcons.__('Add to cart','bravepop').'</a>';


                           //SALE RIBBON
                           $postHTML .= $displayRibbon && $theproduct->is_on_sale() ? '<div class="brave_product_sale_ribon"><span>'.$saleText.'</span><svg preserveAspectRatio="none"  version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="40px" height="40px" viewBox="0 0 40 40" enableBackground="new 0 0 40 40" xmlSpace="preserve"><polygon points="39.998,0.001 -0.002,40.001 -0.002,0" /></svg></div> ' : '';
                           ...
                           if($displayImage) {
                              ...
                              $postHTML .= $layout === 2 ? '<a '.$newWindowHTML.' href="'.get_the_permalink($productID).'">' : '';
                           ...
                           $postHTML .=  '<div class="brave_product__content_wrap">';
                              $postHTML .=  '<div class="brave_product__title"><h4 title="'.$theproduct->get_name().'"><a '.$newWindowHTML.' href="'.get_the_permalink($productID).'">' . $theproduct->get_name() . '</a></h4></div>';
                              ...
                              if($displayCat){
                                 ...
                                 //PRICE
                                 $postPrice = $theproduct->is_on_sale() ? $theproduct->get_sale_price() : $theproduct->get_regular_price();
                                 $postHTML .= $price && $postPrice ?'<div class="brave_product__price">'.$currency . $postPrice.'</div>': '';

                                 $postHTML .=  $displayContent ?'<div class="brave_product__content__excerpt">'.$theproduct->get_short_description().'</div>' : '';
                              $postHTML .=  '</div>';
      ...
      public function render( ) { 
         $layout = isset($this->data->layout) ? $this->data->layout : 1;
         return '<div id="brave_element-'.$this->data->id.'" class="brave_element brave_element--wooProducts">
