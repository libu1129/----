...
function brave_loginUser(event, elementID, popupID){
   ...
   var redirectURL = document.getElementById('brave_login_redirect_'+elementID).value;
   var mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
   
   if(!userEmail){ return brave_show_loginError(elementID, brave_global.email_required);  }
   if(userEmail && !userEmail.match(mailformat)){ return brave_show_loginError(elementID, brave_global.email_invalid);   }
   if(!userPassword){ return brave_show_loginError(elementID, brave_global.pass_required);  }
   if(userPassword && userPassword.length < 5){ return brave_show_loginError(elementID, brave_global.pass_short);   }
   if(!ajaxurl || !security){ return brave_show_loginError(elementID, brave_global.login_error);  }
...
function brave_signupUser(event, elementID, popupID){
   event.preventDefault();
   ...
   var userPassword = document.getElementById('brave_signup_pass_'+elementID).value;
   ...
   var mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
   //Verification
   if(!userFirstname){ return brave_show_loginError(elementID, brave_global.fname_required);  }
   if(!userLastname){ return brave_show_loginError(elementID, brave_global.lname_required);   }
   if(document.getElementById('brave_signup_username_'+elementID) && !username){ return brave_show_loginError(elementID, brave_global.username_required);   }
   if(!userEmail){ return brave_show_loginError(elementID, brave_global.email_required);  }
   if(userEmail && !userEmail.match(mailformat)){ return brave_show_loginError(elementID, brave_global.email_invalid);   }
   if(!userPassword){ return brave_show_loginError(elementID, brave_global.pass_required);   }
   if(userPassword && userPassword.length < 5){ return brave_show_loginError(elementID, brave_global.pass_short);   }
   ...
   var newUserData = { username: username || false, email: userEmail, password: userPassword, sgnupsecurity: security, firstname: userFirstname, lastname: userLastname, redirect: redirectURL, action: 'bravepop_ajax_signup' };
   var isSignupGoal = brave_popup_data[popupID] && brave_popup_data[popupID].settings.goalAction && brave_popup_data[popupID].settings.goalAction.elementIDs[brave_currentDevice]  && brave_popup_data[popupID].settings.goalAction.elementIDs[brave_currentDevice] === elementID;
   //Goal Data
   if(isSignupGoal){
...
function brave_resetPass(event, elementID){
   ...
   brave_ajax_send(ajaxurl, resetpassData, function(status, sentData){
      ...
      if(brave_login_loader){  brave_login_loader.classList.remove('brave_login_loading--show'); }
      if(sentData.sent){
         return brave_show_loginError(elementID, brave_global.pass_reset_success, true); 
      }else{
         return brave_show_loginError(elementID, sentData.message); 
      }
   });
